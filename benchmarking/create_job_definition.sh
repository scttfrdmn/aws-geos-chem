#!/bin/bash
# create_job_definition.sh - Create AWS Batch job definition for GEOS-Chem benchmarking
...
echo "Creating job definition for Architecture: ${ARCHITECTURE}, Processor: ${PROCESSOR}"
...
if [[ "${PROCESSOR}" == "graviton" ]]; then
  JOB_DEF_NAME="geos-chem-benchmark-graviton"
  IMAGE_TAG="benchmark-arm64"
elif [[ "${PROCESSOR}" == "intel" ]]; then
  JOB_DEF_NAME="geos-chem-benchmark-intel-new"
  IMAGE_TAG="benchmark-amd64"
else
  JOB_DEF_NAME="geos-chem-benchmark-amd-new"
  IMAGE_TAG="benchmark-amd64"
fi
...
echo "Job definition ${JOB_DEF_NAME} created successfully."