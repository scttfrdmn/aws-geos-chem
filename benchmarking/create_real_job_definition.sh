#!/bin/bash
# create_real_job_definition.sh - Create AWS Batch job definition for real GEOS-Chem benchmarking
...
echo "Creating real benchmark job definition for Architecture: ${ARCHITECTURE}, Processor: ${PROCESSOR}"
...
if [[ "${PROCESSOR}" == "graviton" ]]; then
  JOB_DEF_NAME="geos-chem-benchmark-graviton-real"
  IMAGE_TAG="benchmark-dynamic-fixed-arm64"
elif [[ "${PROCESSOR}" == "intel" ]]; then
  JOB_DEF_NAME="geos-chem-benchmark-intel-real"
  IMAGE_TAG="benchmark-dynamic-fixed-amd64"
else
  JOB_DEF_NAME="geos-chem-benchmark-amd-real"
  IMAGE_TAG="benchmark-dynamic-fixed-amd64"
fi
...
# Register job definition with increased memory and timeout
aws batch register-job-definition \
  ...
  --timeout '{"attemptDurationSeconds": 10800}'  # 3 hours timeout

echo "Real benchmark job definition ${JOB_DEF_NAME} created successfully."